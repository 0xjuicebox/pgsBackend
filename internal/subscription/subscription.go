...
func (sr SubscriptionResource) CreateOrUpdate(w http.ResponseWriter, r *http.Request) {
	...
	// 2. Set defaults if the frontend omitted them
	if sub.ScheduleType == "" {
		sub.ScheduleType = "daily"
	}
	if len(sub.ActiveDays) == 0 {
		sub.ActiveDays = []int{0, 1, 2, 3, 4, 5, 6} // Default to everyday
	}
	if sub.AnchorDate == "" {
		sub.AnchorDate = time.Now().Format("2006-01-02") // Default anchor to today
	}

	// 3. The Upsert SQL Query
	// Because customer_id is UNIQUE in this table, we can cleanly overwrite existing schedules
	query := `
